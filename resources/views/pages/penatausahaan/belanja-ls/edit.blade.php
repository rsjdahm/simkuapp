<form method="post" action="{{ route('belanja-ls.update', $belanja_ls->id) }}">
    @csrf
    @method('put')
    <div class="row">
        <div class="col-md-6">
                    ...
                    <div class="form-check">
                        <label class="form-check-label">
                            <input type="radio" class="form-check-input" name="status_pending"
                                value="{{ $status_pending }}" @checked($belanja_ls->status_pending == $status_pending)>
                            {{ $status_pending }}
                        </label>
                    </div>
                @endforeach
                                        ...
                                        @foreach ($rek_rincian_objek_item->rek_sub_rincian_objek->sortBy('kode_lengkap') as $rek_sub_rincian_objek_item)
                                            ...
                                            @foreach ($rek_sub_rincian_objek_item->belanja_rka_pd->sortBy('kode_lengkap') as $belanja_rka_pd_item)
                                                <option style="padding-left: 3rem;"
                                                    ...
                                                    @selected($belanja_ls->belanja_rka_pd_id == $belanja_rka_pd_item->id)>
            ...
            <div class="form-group">
                <label class="form-label">Nomor <span class="text-warning">*</span></label>
                <input autofocus name="nomor" class="form-control" value="{{ $belanja_ls->nomor }}">
            </div>
            <div class="form-group">
                <label class="form-label">Tanggal <span class="text-warning">*</span></label>
                <input type="date" name="tanggal" class="form-control" value="{{ $belanja_ls->tanggal }}">
            </div>
            <div class="form-group">
                <label class="form-label">Uraian <span class="text-danger">*</span></label>
                <textarea name="uraian" class="form-control">{{ $belanja_ls->uraian }}</textarea>
            </div>
            <div class="form-group">
                <label class="form-label">Nilai <span class="text-danger">*</span></label>
                <input type="number" name="nilai" step=".0001" class="form-control"
                    value="{{ $belanja_ls->nilai }}">
            </div>
            <div class="form-group">
                ...
                <select name="metode_pembayaran" class="form-control">
                    ...
                    @foreach (\App\Enums\Penatausahaan\MetodePembayaran::cases() as $metode_pembayaran)
                        <option value="{{ $metode_pembayaran }}" @selected($belanja_ls->metode_pembayaran == $metode_pembayaran)>{{ $metode_pembayaran }}
                        </option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                ...
                <select name="status" class="form-control">
                    @foreach (\App\Enums\Penatausahaan\StatusPosting::cases() as $status)
                        <option value="{{ $status }}" @selected($belanja_ls->status == $status)>{{ $status }}</option>
                    @endforeach
                </select>
            </div>
        </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <input name="nama" class="form-control" value="{{ $belanja_ls->nama }}">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="alamat" class="form-control">{{ $belanja_ls->alamat }}</textarea>
                    </div>
                    <div class="form-group">
                        <label class="form-label">NPWP</label>
                        <input type="number" name="npwp" class="form-control" value="{{ $belanja_ls->npwp }}">
                    </div>
                    <div class="form-group">
                        ...
                        <select name="bank_id" class="form-control">
                            ...
                            @foreach ($bank as $bank_item)
                                <option value="{{ $bank_item->id }}" @selected($belanja_ls->bank_id == $bank_item->id)>
...
<script>
    $("form[action='{{ route('belanja-ls.update', $belanja_ls->id) }}']").on("submit", function(event) {
