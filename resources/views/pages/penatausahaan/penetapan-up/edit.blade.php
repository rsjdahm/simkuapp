<form method="post" action="{{ route('penetapan-up.update', $penetapan_up->id) }}">
    @csrf
    @method('put')
    <div class="row">
        <div class="col-md-6">
            <div class="form-group">
                ...
                <select name="sub_unit_kerja_id" class="form-control">
                    ...
                    @foreach ($sub_unit_kerja as $sub_unit_kerja_item)
                        <option value="{{ $sub_unit_kerja_item->id }}" @selected($penetapan_up->sub_unit_kerja_id == $sub_unit_kerja_item->id)>
            ...
            <div class="form-group">
                ...
                <select name="rek_sub_rincian_objek_id" class="form-control">
                    ...
                    @foreach ($rek_sub_rincian_objek as $rek_sub_rincian_objek_item)
                        <option value="{{ $rek_sub_rincian_objek_item->id }}" @selected($penetapan_up->rek_sub_rincian_objek_id == $rek_sub_rincian_objek_item->id)>
            ...
            <div class="form-group">
                ...
                <input autofocus name="nomor" class="form-control" value="{{ $penetapan_up->nomor }}">
            </div>
        </div>
        <div class="col-md-6">
            <div class="form-group">
                <label class="form-label">Tanggal</label>
                <input type="date" name="tanggal" class="form-control" value="{{ $penetapan_up->tanggal }}">
            </div>
            <div class="form-group">
                <label class="form-label">Uraian</label>
                <textarea name="uraian" class="form-control">{{ $penetapan_up->uraian }}</textarea>
            </div>
            <div class="form-group">
                <label class="form-label">Nilai</label>
                <input type="number" name="nilai" step="0.0001" class="form-control"
                    value="{{ $penetapan_up->nilai }}">
            </div>
            <div class="form-group">
                <label class="form-label">Status</label>
                <select name="status" class="form-control">
                    @foreach (\App\Enums\Penatausahaan\StatusPosting::cases() as $status)
                        <option value="{{ $status }}" @selected($penetapan_up->status == $status)>{{ $status }}</option>
                    @endforeach
                </select>
            </div>
        </div>
...
<script>
    $("form[action='{{ route('penetapan-up.update', $penetapan_up->id) }}']").on("submit", function(event) {
