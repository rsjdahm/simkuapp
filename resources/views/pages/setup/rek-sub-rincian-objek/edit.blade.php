<form method="post" action="{{ route('rek-sub-rincian-objek.update', $rek_sub_rincian_objek->id) }}">
    @csrf
    @method('put')
                                ...
                                @foreach ($rek_objek as $rek_objek_item)
                                    @if ($rek_objek_item->rek_jenis_id == $rek_jenis_item->id)
                                        ...
                                        @foreach ($rek_rincian_objek as $rek_rincian_objek_item)
                                            @if ($rek_rincian_objek_item->rek_objek_id == $rek_objek_item->id)
                                                <option @selected($rek_rincian_objek_item->id == $rek_sub_rincian_objek->rek_rincian_objek_id) style="padding-left: 3rem;"
                                                    value="{{ $rek_rincian_objek_item->id }}">
    ...
    <div class="form-group">
        ...
        <input type="number" name="kode" class="form-control" value="{{ $rek_sub_rincian_objek->kode }}">
    </div>
    <div class="form-group">
        <label class="form-label">Uraian Rekening Objek</label>
        <textarea name="nama" class="form-control">{{ $rek_sub_rincian_objek->nama }}</textarea>
    </div>
    <div class="form-group">
...
<script>
    $("form[action='{{ route('rek-sub-rincian-objek.update', $rek_sub_rincian_objek->id) }}']").on("submit", function(
        event) {
