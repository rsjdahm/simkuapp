    ...
    public function index(Builder $builder, Request $request)
    {
        if ($request->rek_akun_id) {
            if ($request->wantsJson()) {
                ...
                return DataTables::of($data)
                    ...
                    ->addColumn('detail', function ($item) {
                        return '
                        <div class="btn-group btn-group-sm">
                            <a do="open-to-tab" target="rek_jenis_page" tab="#rek_jenis" href="' . route('rekening.rek_jenis.index', ['rek_kelompok_id' => $item->id]) .  '" class="btn btn-primary text-white"><i class="fas fa-forward"></i></a>
                        </div>
                        ';
                    })
            ...
            $table = $builder->ajax(route('rekening.rek_kelompok.index', ['rek_akun_id' => $request->rek_akun_id]))
                ...
                ->addColumn(['data' => 'kd_rek', 'title' => 'Kode Kelompok', 'class' => 'font-weight-bold', 'style' => 'width: 1%;'])
