    ...
    public function index(Builder $builder, Request $request)
    {
        if ($request->rek_objek_id) {
                ...
                return DataTables::of($data)
                    ...
                    ->addColumn('action', function ($item) {
                        ...
                        <div class="btn-group btn-group-sm">
                            <a data-load="modal" title="Edit Rekening Rincian Objek" href="' . route('rekening.rek_rinc_objek.edit', $item->id) . '" class="btn btn-warning text-white"><i class="fas fa-edit"></i></a>
                            <a data-action="delete" href="' . route('rekening.rek_rinc_objek.destroy', $item->id) . '" class="btn btn-danger text-white"><i class="fas fa-trash"></i></a>
                        </div>
                        ';
                    })
                    ->addColumn('detail', function ($item) {
                        return '
                        <div class="btn-group btn-group-sm">
                            <a data-action="open-tab" data-target="#rek_sub_rinc_objek" href="' . route('rekening.rek_sub_rinc_objek.index', ['rek_rinc_objek_id' => $item->id]) . '" class="btn btn-primary text-white"><i class="fas fa-forward"></i></a>
                        </div>
                        ';
                    })
            ...
            $table = $builder->ajax(route('rekening.rek_rinc_objek.index', ['rek_objek_id' => $request->rek_objek_id]))
                ...
                ->addColumn(['data' => 'kd', 'title' => 'Kode Rincian Objek', 'class' => 'font-weight-bold text-nowrap', 'style' => 'width: 1%;'])
