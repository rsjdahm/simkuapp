...
use App\Http\Requests\Parameter\Global\Rekening\RekJenisRequest;
    ...
    public function index(Builder $builder, Request $request)
    {
        if ($request->rek_kelompok_id) {
                ...
                return DataTables::of($data)
                    ...
                    ->addColumn('action', function ($item) {
                        ...
                        <div class="btn-group btn-group-sm">
                            <a load="modal" title="Edit Rekening Jenis" href="' . route('rekening.rek_jenis.edit', $item->id) . '" class="btn btn-warning text-white"><i class="fas fa-edit"></i></a>
                            <a do="delete" href="' . route('rekening.rek_jenis.destroy', $item->id) . '" class="btn btn-danger text-white"><i class="fas fa-trash"></i></a>
                        </div>
                        ';
                    })
                    ->addColumn('detail', function ($item) {
                        return '
                        <div class="btn-group btn-group-sm">
                            <a do="open-to-tab" target="rek_objek_page" tab="#rek_objek" href="' . route('rekening.rek_objek.index', ['rek_jenis_id' => $item->id]) .  '" class="btn btn-primary text-white"><i class="fas fa-forward"></i></a>
                        </div>
                        ';
                    })
            ...
            $table = $builder->ajax(route('rekening.rek_jenis.index', ['rek_kelompok_id' => $request->rek_kelompok_id]))
                ...
                ->addColumn(['data' => 'kd_rek', 'title' => 'Kode Jenis', 'class' => 'font-weight-bold', 'style' => 'width: 1%;'])
